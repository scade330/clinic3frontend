...
export default function PharmacyDialogForm({ 
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const apiEndpoint = isEditing ? `/api/pharmacy/${itemToEdit._id}` : "/api/pharmacy";
            const apiMethod = isEditing ? axios.put : axios.post;
            // -------------------------------

            const response = await apiMethod(apiEndpoint, formData);
            
            toast.success(`Pharmacy item ${isEditing ? "updated" : "created"} successfully!`);
    ...
    const formFields = [
        "itemName", "category", "quantityInStock", "reorderLevel", "costPrice", 
        "sellingPrice", "batchNumber", "expiryDate", "supplier", "purchaseDate", 
        "location"
    ];
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                {propOpen === undefined && (
                    <Button className="bg-blue-600 hover:bg-blue-700">{buttonTitle}</Button>
                )}
            </DialogTrigger>
            ...
            <DialogContent className="bg-white p-6 rounded-lg shadow-lg max-w-lg mx-auto overflow-y-auto max-h-[90vh]">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
                    {formFields.map((field) => (
                        <div key={field} className={["itemName", "supplier", "batchNumber", "location"].includes(field) ? "col-span-2" : "col-span-1"}>
                            <Label htmlFor={field}>{formatLabel(field)}</Label>
                            <Input
                                type={["expiryDate", "purchaseDate"].includes(field) ? "date" : 
                                      ["quantityInStock", "reorderLevel", "costPrice", "sellingPrice"].includes(field) ? "number" : "text"}
                                id={field}
                                name={field}
